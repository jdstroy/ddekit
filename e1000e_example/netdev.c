...
e1000_print_ring(struct e1000_ring *ring)
{
        printk(KERN_INFO "%s: debugging ring @%p, dma addr: 0x%lx, size: %d, no desc: %d\n", __FUNCTION__,
                (void*)ring->desc, (unsigned long)ring->dma, ring->size, ring->count);
        printk(KERN_INFO "%s: head: %d, tail: %d, unused desc: %d, next_to_use: %d, next_to_clean %d\n", __FUNCTION__,
                ring->head, ring->tail, e1000_desc_unused(ring), ring->next_to_use, ring->next_to_clean);
	print_hex_dump(KERN_DEBUG, "", DUMP_PREFIX_OFFSET, 16, 2, ring->desc, ring->count*sizeof(struct e1000_rx_desc), 0);
	printk(KERN_INFO "%s: end of debug for %s\n", __FUNCTION__, ring->name);
}

void
e1000_debug_receive_ring(struct e1000_adapter *adapter)
{
        struct e1000_hw *hw = &adapter->hw;

        printk("RDBAH/L: 0x %08x %08x  ", er32(RDBAH(0)), er32(RDBAL(0)));
        printk("TDBAH/L: 0x %08x %08x\n", er32(TDBAH(0)), er32(TDBAL(0)));
        printk("RDLEN:   0x %08x           ", er32(RDLEN(0)));
        printk("TDLEN:   0x %08x\n", er32(TDLEN(0)));
        printk("RDH:     0x %08x           ", er32(RDH(0)));
        printk("TDH:     0x %08x\n", er32(TDH(0)));
        printk("RDT:     0x %08x           ", er32(RDT(0)));
        printk("TDT:     0x %08x\n", er32(TDT(0)));
}

void
e1000_print_rx_desc(struct e1000_adapter *adapter, struct e1000_rx_desc *desc)
{
        ...
        for(ring = 0; ring < rx_ring->count; ring++) {
                if(rx_ring->buffer_info[ring].dma == desc->buffer_addr) {
                        printk("%s: found buffer info for addr 0x%llx at %p\n", __FUNCTION__, desc->buffer_addr,
                                (void*)rx_ring->buffer_info[ring].skb->data);
                        buf = (u8*)rx_ring->buffer_info[ring].skb->data;
                }
        }
        max = (desc->length > 128) ? 128 : desc->length;
        printk("printing DMA buffer:\n");
        print_hex_dump(KERN_DEBUG, "rx_cont: ", DUMP_PREFIX_OFFSET, 16, 2, buf, max, 0);
        printk("\nEnd of DMA buffer\n");
}
...
/* detect rx unit hang */
#if 0
	static long last_run = -1UL;
	static int last_desc = 0;
	if(time_after(jiffies, last_run + 3*HZ)) {
		e1000e_dump(adapter);
		e1000_print_ring(adapter->rx_ring);
	}
	if(i != last_desc) {
		last_run = jiffies;
		last_desc = i;
...
/* *INDENT-ON* */
	...
	adapter->msg_enable = (1 << NETIF_MSG_DRV | NETIF_MSG_PROBE) - 1 | NETIF_MSG_HW;
